...
public class ParkingStatus extends AppCompatActivity {
    ...
    public void open_details(View view){
        ...
        data = mDbHelper.fetchCarInLot(getCarLot((Button) view));
    ...
    public void fillData()
    {
        ...
        for(int i=1;i<=50;++i)
        {
            if(isEmpty[i] == 1)
            {
                int lot = getCarLot(btns[i]);
                if(lot%2 == 1)
                    btns[i].setBackgroundResource(R.drawable.empty_lot);
                else
                    btns[i].setBackgroundResource(R.drawable.empty_lot2);
                btns[i].setText(Integer.toString(lot));
